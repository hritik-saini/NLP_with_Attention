...
class TokenizedDataStream:
    ...
    def create_tokenizer(self, lines, vocab_size):
        ...
        lines_with_eos = [line + ' <EOS>' for line in lines]
        tokenizer.fit_on_texts(lines_with_eos)
        return tokenizer
    ...
    def get_tokenized_stream(self):
        ...
        # check -- input is pt and output is english
        return self.append_eos(pt_sequences, en_sequences)

    def detokenize(self, integers, type): ## What the fuck type means here -- check that again
        integers = list(np.squeeze(integers))
        print(integers)
        if self.EOS in integers:
            integers = integers[:integers.index(self.EOS)]
